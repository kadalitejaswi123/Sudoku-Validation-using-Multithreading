...
void* validateChunk(void* arg) {
    ThreadArgs* args = (ThreadArgs*)arg;
    bool local_valid = true;
    int* segment = (int*)malloc(args->N * sizeof(int));

    for (int idx = args->start_idx; idx < args->end_idx && local_valid; idx++) {
...
int main() {
    ...
    for (int type = 0; type < 3; type++) {
        int chunk_size = N / threads_per_type;
        for (int i = 0; i < threads_per_type; i++) {
            int thread_idx = type * threads_per_type + i;
            thread_args[thread_idx].thread_id = thread_idx + 1;
            thread_args[thread_idx].start_idx = i * chunk_size;
            thread_args[thread_idx].end_idx = (i == threads_per_type - 1) ? N : (i + 1) * chunk_size;
            ...
            pthread_create(&threads[thread_idx], NULL, validateChunk, &thread_args[thread_idx]);
        }
    }
    ...
    double time_taken = (e.QuadPart - s.QuadPart) * 1e6 / fre.QuadPart;
